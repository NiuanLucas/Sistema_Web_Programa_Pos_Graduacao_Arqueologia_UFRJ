...
//INSERIR NO BANCO DE DADOS
    if(isset($_POST["slide_descricao"])){
      ...
      for ($i = 1; $i <= $slide_tamanho; $i++) {
        $legenda = base64_encode($_POST["legenda_".$i]);

        $texto2_slide .= ", legenda_".$i."";
        $texto2_var_slide .= "'".$legenda."'";
        $texto2_final .=  $texto2_slide. " = " .$texto2_var_slide;
        $texto2_slide = "";
        $texto2_var_slide = "";
...
<div class="col-sm-6">
    ...
    <label for="exampleFormControlInput1">Legenda da Imagem #<?php echo $i; ?></label>
    <textarea rows="5" id="legenda_<?php echo $i ?>" name="legenda_<?php echo $i ?>" class="form-control ckeditor"> <?php echo base64_decode($dados_pagina['legenda_'.$i]); ?> </textarea>
    </div> 
    </div>
    ...
    <input value="<?php echo $dados_pagina['slide_'.$i]; ?>" type="text" name="pagina_modular_imagem_capa_<?php echo $i ?>" class="form-control" placeholder="">
    </div>
    ...
    <script>
     CKEDITOR.replace( 'legenda_<?php echo $i ?>', {
      height: 100,
